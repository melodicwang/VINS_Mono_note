...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    // 此处为基于范围的for循环（C++11），auto类型也为C++11新标准中的，用来自动获取变量的类型
    // for述句的第一部分定义被用来做范围迭代的变量，就像被声明在一般for循环的变量一样，其作用域仅限于循环的范围。
    // 冒号:之后的第二部分，代表将被迭代的范围。
    // 将 estimator.f_manager.feature 中的值依次赋值给 it_per_id ，每赋值一次执行一次循环
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        int used_num;
        used_num = it_per_id.feature_per_frame.size();
        // && 逻辑“与” 两边均为真时为真。如果第一个为 false 第二个表达式不计算
        // 此处意为除非括号中两个条件均满足，否则 continue
