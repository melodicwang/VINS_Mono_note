...
class IntegrationBase
{
  public:
    ...
    void midPointIntegration(double _dt,                    // 输入:当前IMU数据和前一个IMU数据的时间差
                            const Eigen::Vector3d &_acc_0,  // 前一个IMU加速度计数据
                            const Eigen::Vector3d &_gyr_0,  // 前一个IMU陀螺仪数据
                            const Eigen::Vector3d &_acc_1,  // 当前IMU加速度计数据
                            const Eigen::Vector3d &_gyr_1,  // 当前IMU陀螺仪数据
                            const Eigen::Vector3d &delta_p,     // 前一个IMU 平移预计分测量值
                            const Eigen::Quaterniond &delta_q,  // 前一个IMU 旋转预积分测量值
                            const Eigen::Vector3d &delta_v,     // 前一个IMU 速度预积分测量值
                            const Eigen::Vector3d &linearized_ba, // 前一个ba和bg
                            const Eigen::Vector3d &linearized_bg,
                            Eigen::Vector3d &result_delta_p,        // 输出:当前IMU预计分测量值
                            Eigen::Quaterniond &result_delta_q, 
                            Eigen::Vector3d &result_delta_v,
                            Eigen::Vector3d &result_linearized_ba,  // 输出:当前ba和bg
