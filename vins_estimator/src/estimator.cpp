...
void Estimator::optimization()
{
    ...
    loss_function = new ceres::CauchyLoss(1.0);     // 设置柯西损失函数因子（核函数，用来减小Outlier的影响）
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        ...
        problem.AddParameterBlock(para_SpeedBias[i], SIZE_SPEEDBIAS); // ParameterBlock：优化变量
    }

    // Step1.2：camera IMU的外参也添加到估计 [p_cb,q_cb](7)
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
    ...
    marginalization的结构是始终存在的，随着下面marginazation的结构更新，last_marginalization_parameter_blocks
    对应的是还在滑窗中的变量。这里可以这样理解，下面会添加对IMU和视觉的残差，但是，这些对应的变量实际上跟之前被margin掉
    的变量是有约束的，这里的last_marginalization_parameter_blocks就是保存和被边缘化的变量有约束关系的这些变量，也就是
    heyijia博客中对应的Xb变量，last_marginalization_info中对应的是Xb对应的测量Zb，这里用先验来表示这个约束，整个margin
    部分实际上就是在维护这个结构。关于这一部分的理解，http://blog.csdn.net/heyijia0327/article/details/53707261 */
